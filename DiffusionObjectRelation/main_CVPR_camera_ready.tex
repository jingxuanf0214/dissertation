...
% \def\paperID{21274} % *** Enter the Paper ID here
\def\paperID{43000} % *** Enter the Paper ID here
\def\confName{CVPR}
\def\confYear{2026}

%%%%%%%%% TITLE - PLEASE UPDATE
\title{Circuit Mechanisms for Spatial Relation Generation in Diffusion Transformers}

%%%%%%%%% AUTHORS - PLEASE UPDATE
\author{Binxu Wang\thanks{equal contribution}\\
Kempner Institute, Harvard University\\
{\tt\small binxu\_wang@hms.harvard.edu}
...
\and
Jingxuan Fan\footnotemark[1]\\
Harvard University\\
%First line of institution2 address\\
{\tt\small jfan@g.harvard.edu}
\and
Xu Pan\footnotemark[1]\\
Harvard University\\
%First line of institution2 address\\
{\tt\small xupan@fas.harvard.edu}
}
...
%Although mechanistic interpretability approaches have been adopted to explain neural networks’ behavior in language and vision transformers from the perspective of the internal computation of representations, they have not yet been used to study how a DiT can generate correct spatial relations between objects.
\end{abstract}


\section{Introduction} 

% \binxu{Training details and methods for reproducibility}

% \binxu{Add relation head heatmap of other trained models. "The claim at L220 "We found cross attention head with this property consistently in DiT models of three different scales (DiT-B, mini, micro) we trained" lacks supporting evidence: readers must trust the authors' word. Why not include plots similar to Figure 4 in the appendix for other models?"}
...
% We recognize our results in three sections.

% 1) our setup. RTD and T5 both succeed in generating correct relation. We additionally show learning dynamics [generation dynamics? I have an ablation figure for RTE can do for T5].

% 2) We dig into the RTD circuits.

% 3) We dig into the T5 circuits.

% \begin{wrapfigure}[23]{r}{0.45\linewidth}
\begin{figure}[!htbp]
  \centering
  \vspace{-2pt}
  \includegraphics[width=\linewidth]{figs/Data_schematics-03.png}
...
\section{Background}
%\paragraph{Denoising diffusion model}
\paragraph{Spatial generation failure in T2I models}
Failure in multi-object spatial relation generation has been widely reported in T2I models. While new models improved significantly in generating accurate single object attributes, the improvement of generating correct relations between objects is less impressive (Fig.~\ref{fig:eval_T2I}). One common view is that spatially localized cross-attention grounds object placement. Building on this hypothesis, recent work tackles spatial-relation failures by directly manipulating attention at inference. \cite{chefer2023attendandexciteattentionbasedsemanticguidance} proposes to specifically enhance the cross-attention to subject tokens to prevent catastrophic neglect and improve attribute binding; \cite{phung2023groundedtexttoimagesynthesisattention} optimizes cross- and self-attention using layout-derived losses—via user boxes or LLM-proposed layouts—to enforce multi-object placement and spatial relations. On the other hand, works like \cite{zhang2024compassenhancingspatialunderstanding} and \cite{kang2025clipidealnofix} argue that the poor spatial performance of T2I models stems from the limitations of text encoders. \cite{zhang2024compassenhancingspatialunderstanding} finds that text encoders used across frontier T2I models do not sufficiently preserve spatial relations information in their encoding. Similarly, \cite{kang2025clipidealnofix} argues that multiple image properties including spatial relations cannot be simultaneously encoded by CLIP. Our work is unique in that we unite these two views: we show that different information encoding from text-encoders contribute to distinctive attention patterns and circuits. 

% These works focus the problem on certain limited stages of the T2I generation, thereby also leading to different remedy solutions. Our work differs in that we mechanistically dissect the generation mechanism across stages by varying text encoding, identifying spatial layout circuit and single object generation circuit respectively. Our results suggest that a generalizable spatial relation circuit crucially depends on text encoding information, bridging different views from prior literature. 

%Prompt-to-Prompt \cite{hertz2022prompttopromptimageeditingcross} edits or transfers cross-attention maps to keep composition stable under prompt changes;  
%Work that tackles spatial relations in T2I via attention guidance primarily manipulates cross-attention at inference to coerce layouts: \cite{hertz2022prompttopromptimageeditingcross} copies or edits cross-attention maps to preserve composition under prompt changes; \cite{chefer2023attendandexciteattentionbasedsemanticguidance} excites subject tokens so all entities appear; and \cite{phung2023groundedtexttoimagesynthesisattention} explicitly optimizes cross- and self-attention toward a target layout, improving multi-object placement. While effective for control, these methods largely use attention as a knob rather than explain how spatial predicates are implemented internally. Moreover, these methods frequently require injecting additional instructions like layout. On the other hand, interpretability work like \cite{tang2022daam} begins to connect individual cross-attention heads to word–region bindings but stops short of isolating relation-specific mechanisms. Our work takes a circuits-based interpretability approach to specifically isolate relation-specific pathways that compute spatial predicates from text. %e explain the native mechanism that yields correct spatial relation without auxiliary layout inputs.
 
% \paragraph{Diffusion transformer} 
% \citep{Peebles2023Scalable}


\paragraph{Interpretability in diffusion}

The work that shares the most similar interest with us is \citep{Okawa2024Compositional,Park2024Emergence}, which studies the learning dynamics of composition of attributes on a single object in a conditional diffusion model using a minimalist dataset. The key difference is that we focus on the generation of a composition of multiple objects on the scene instead of a single object. Moreover, we study the architecture where the conditioning signal is encoded by multiple word vectors (e.g. T5 encoder) and passed to image tokens via cross attention, instead of a single vector summing all attributes as in \cite{Okawa2024Compositional}. Our setup is more closely related to modern T2I frameworks \citep{chen2023PixArtAlpha,Rombach2022LatentDiffusion,Xie2024SANA}.


%\citep{tang2022daam}

%\citep{liu_towards_2024}

\section{Train T2I models to generate spatial relation}





%Previous work demonstrates that pretrained T2I models across different text encoders and architectures show disproportionally better capabilities for single object feature generations than object 2D (top-bottom, left-right) or 3D (front-back) relations \citep{huang2023t2icompbench}. This observation motivates us to study the mechanism of spatial relation generation and why it frequently fails in T2I models. 
...
% Open source pretrained T2I models often have image-text data distribution and unreliable spatial composition capability, making it hard to study mechanisms reliably.
%Given the unreliable performance of these open-source models on such task, we cannot dissect the circuit for a capability the model does not possess. 
% To study how T2I diffusion models can generate correct spatial relations and why they sometimes fail we first constructed a minimal dataset and trained models from scratch. 
% Given the unreliable performance of open-source pretrained models on such relational composition, we cannot dissect the circuit for a capability the model does not possess. Thus, our first step was to create a setup in which diffusion models reliably learn the skill, making it amenable to mechanistic analysis. The desiderata for this dataset were:
...
% 3) samples simple enough to evaluate rigorously.
% Dissecting how pre-trained text2image (T2I) diffusion models generate spatial relations is challenging: their training distributions are not accessible, and their object-relation behavior is unreliable, hindering circuit-level analysis.
%For pre-trained text2image (T2I) diffusion models, we usually don't have access to their exact training distribution. Further, their performance on the object relation is not reliable, challenging the the circuit dissection of this capability. 
% Then we dissect the minimal model and to find the generalizable circuit and link it back to the pre-trained large scale models. 
% \begin{figure}[!htp]
% \begin{center}
% %\framebox[4.0in]{$\;$}
% \vspace{-10pt}
% \includegraphics[width=.85\textwidth]{figs/Data_schematics-01.png}
% \end{center}
% \vspace{-8pt}
% \caption{\textbf{Dataset set up and model performance}. \textbf{A.} Training dataset and model architecture. \textbf{B.}  Model performance comparison on exact training prompt format \enquote{Obj1 is Relation Obj2}. \textbf{C.} Performance on slight prompt variation \enquote{Obj1 is Relation the Obj2}, adding the dramatically decreased the performance of T5 model but not RTE, RTEP model. 
% }\label{fig:data_schematics}\label{fig:model_perf_cmp}
% \end{figure}
\paragraph{Dataset setup}
We reason that such a dataset should have following properties:
1) multiple objects in the scene with distinct features,
2) objects arranged to satisfy specific spatial relations described by the prompt, and
3) samples are simple enough to be evaluated rigorously. Guided by this principle, we design a dataset of the following format: each sample consists of a prompt in the format \texttt{[descriptor A] [object A] [relation] [descriptor B] [object B]}, e.g., "\textit{red square above and to the left of blue circle}", and a corresponding image with two objects positioned on a gray background (Fig.~\ref{fig:diagram}\textbf{A}). %To this purpose, we designed a dataset of paired images and prompts and trained a text-to-image diffusion model with the PixArt framework. 
...
The color descriptors A and B are randomly dropped, and spatial relations are described with multiple paraphrases to add variability (details in App.~\ref{method:dataset}). 

% \begin{wrapfigure}[22]{r}{0.6\linewidth}
% \begin{center}
% %\framebox[4.0in]{$\;$}
% \vspace{-10pt}
% \includegraphics[width=0.60\textwidth]{figs/Data_schematics-02.png}
% \end{center}
% \vspace{-10pt}
% \caption{\textbf{Training dynamics}. \textbf{A.} Training dataset and model architecture. \textbf{B.}  Model performance comparison. \textbf{C.} Training dynamics of sample accuracy. }\label{fig:DiT_training_dyn_cmp}
% \end{wrapfigure}

% \begin{wrapfigure}[25]{r}{0.60\linewidth}
\begin{figure}[!htbp]
  \centering
  \vspace{-5pt}
  \includegraphics[width=\linewidth]{figs/Data_schematics_Final.png}
  \vspace{-10pt}
  \caption{\textbf{Training dynamics of the T2I models (DiT-B)}. \textbf{A.} and \textbf{B.} Both models trained with random token embedding (RTE) and T5 can achieve good accuracy on the task. Solid lines shows the result of model using exponential moving averaged (\texttt{ema}) weights, while dashed line shows the non-averaged weights. 
  \textbf{C.} The task is learned in distinct stages. In both models, they first learn to generate objects but with wrong attributes binding, then they the correct binding of single-object attributes (e.g. red square), finally they learn the correct spatial relation.
  (see App.~\ref{app:eval_learn_dynamics} for other models). 
  }
  \label{fig:dynamics}
  \vspace{-5pt}
\end{figure}
...
This comparison tests whether the diffusion transformer can learn object relations without semantic or contextual structure in the text embeddings, enabling us to localize the model’s relational computations (Fig.~\ref{fig:diagram}\textbf{A}). (details in App.~\ref{method:model_arch}). %and diffusion operates in a lower resolution latent space ($4\times16\times 16$). %Training uses AdamW for 160{,}000 steps (Fig.~\ref{fig\data_schematics}\textbf{A}).

%For text conditioning, we tested a few options for encoding, including 1) the T5-XXL text encoder \citep{Raffel2023T5Model}, 2) random token encoder with position embedding (\textit{RTEP}) or 3) without position embedding (\textit{RTE}). 
%For random token encoder (RTE), we created a dictionary mapping each token id onto a 4096d random vector with norm similar to T5; and for RTEP, we add extra sinusoidal position encoding onto it. 
%Comparison between these encoders can reveal whether diffusion transformer itself can learn object relations without \textit{semantic} or \textit{contextual} structure in the text embeddings, which helps locating the relational circuits. %\binxu{Add more} 
% We used VAE to compress the sample space to 16 x 16, which leads to 8x8 tokens at patch size 2. 
%In line with pretrained models, we spatially compressed samples with a variational auto-encoder (VAE) pretrained in Stable Diffusion \citep{Rombach2022LatentDiffusion} and then diffusion happens in the lower resolution latent space ($4\times16\times 16$). 
%We trained diffusion transformers (DiT) at different scales, DiT-B (12 layer, 12 heads, 768d), mini (6L6H, 384d), micro (6L3H, 192d), nano (3L3H, 192d) with patch size 2, using AdamW optimizers for 160,000 steps. (Fig.~\ref{fig:data_schematics}\textbf{A}). 
% More details for the tokenizer, model architecture and training can be found in App.~\ref{method:architecture}. 


%%%% COULD ADD in final version 
% \subsection{Normative analysis of the circuit}
% \binxu{not sure where this section should be.... should discuss}
% To correctly generate according to relational prompt, the conditional diffusion model needs to tackle a series of challenges. 
% 1) determine where to generate the two objects according to the prompt. 
% 2) generate objects with correct feature binding at each location. 
% 3) make sure there are only two objects on the scene. 

% To some extent, there is a chicken and egg coordination problem:
% If a token doesn't know the location of the other object, how does it know which object it should become? But if the tokens haven't decided what to become, how do they know the spatial relation to the other object. \binxu{polish}
% So should diffusion first decide where to generate or what to generate? 

% Here we seek inspiration from a field with no obvious relation --- \textit{developmental biology}. 
% In the embryo, cells start from nearly identical origin, then by internal interaction or external control signal, they coordinate with each other to become different organs of the body. 
% This process is deeply similar to the generation process of diffusion transformer, where image tokens start from i.i.d. noise, then differentiates into different object in the scene. 
%In embryo, \binxu{Jingxuan can write more about this. }
% The development of the tokens into correct features is like the development of cells in the embryo into organs. 
...
Specifically, we evaluate four aspects of samples: 1) existence of correct colors on the image, denoted as \texttt{color}, 2) existence of correct shapes on the image, denoted as \texttt{shape} 3) correctness of shape color binding on the two objects, denoted as \texttt{unique\_binding} and 4) correctness of spatial relation between the identified two objects, denoted as \texttt{spatial\_relation}. 
Each metric produces a binary (0,1) score for each sample, which we then average to obtain the model’s accuracy.
%(App.~\ref{method:sample_eval}). 
%Metrics cover: (1) presence of the specified colors; (2) presence of the specified shapes; (3) correct color–shape binding for both objects; and (4) correct spatial relation between the two identified objects (App.~\ref{method:sample_eval}). Because (4) requires (3), it serves as overall sample correctness.
%Through out training, we generate samples according to 96 prompts, covering all 8 spatial relations and 12 unique object combinations, using the default sampler (\texttt{DPMsolver++} with 14 steps \citep{lu2022dpm++} and classifier-free guidance $4.5$ \citep{ho2022classifierFreeGuidance}). 
%To evaluate the samples, we used classic segmentation and classification tools from \texttt{cv2}, and evaluate whether and on which aspect they are consistent with the prompts. 
%Here we can evaluate on multiple levels of granularity: 1) existence of correct colors on the image, 2) existence of correct shapes on the image, 3) correctness of shape color binding on the two objects, and 4) correctness of spatial relation between the identified two objects (App.~\ref{method:sample_eval}). 
%Note that for evaluation of 4), we need 3) to be correct, so 4) is the overall correctness of the sample. 

\paragraph{Model comparison} 
Accuracy on all four metrics increases with model parameter sizes up to the DiT-mini configuration; accuracy gains from DiT-mini to DiT-B are marginal (Tab. \ref{tab:model_cmp_eval_table}). All trained models at the largest parameter size (DiT-B) show high accuracy in \texttt{color} and \texttt{shape}, but the \texttt{unique\_binding} and\texttt{spatial\_relation} accuracy varies significantly depending on the chosen text encoders (Tab. \ref{tab:model_cmp_eval_table}). % Fig.~\ref{fig:model_perf_cmp}\textbf{B}, 
%First, position of token encoding is key. 
...
However, RTE without positional encoding is significantly worse on these metrics. %This failure can be explained by the permutation-invariant property of cross-attention. 
...
%This highlights the \textit{non-commutativeness} of complex prompts with multiple objects and spatial relations, where simple vector addition is insufficient \citep{Wattenberg2024Relational}; by contrast, in single-object settings, feature addition is commutative \cite{Okawa2024Compositional}.

 
%Both RTEP- and T5-trained models improve spatial accuracy with larger model scale, but RTEP consistently achieves higher relational accuracy than T5 across all parameter scales. 
%These results suggest that semantic/contextual embeddings from a pretrained LM are not strictly necessary for DiTs to learn object relations and may, in some settings, impede relational learning. 
%This highlights that, semantic and contextual word embedding from pre-trained language model is not necessary for DiT to learn to generate correct object relation, it could even be harmful for learning relation!
% The model with both T5 and random embedding with position encoding achieved high performance in generating according to the text prompt (xxx\%). 


%The failure of position-free random embedding is intuitive: the cross attention output is invariant to token permutation, without position embedding, "red A on top of blue B", "blue B on top of red A" and "blue A on top of red B" will exactly be the same \textit{bag of words} and yield the same generation results. 
%Thus, feature binding have at most 50\% chance to be correct, and on top of that spatial relation have 50\% chance to be the intended one (Fig.~\ref{fig:model_perf_cmp}). 
% Specifically, color binding will fail and the spatial relation will be symmetric (Fig.~\ref{fig:model_perf_cmp}). 
%This also demonstrates the \textit{non-commutativeness} of complex language prompt, especially when multiple objects and spatial relation are present, and a simple vector summation of token embeddings cannot solve this \citep{Wattenberg2024Relational}. This is different from the single object case as studied in \cite{Okawa2024Compositional}, where addition of feature is commutative. %such non-commutativeness does not exist. 
% of the spatial relation, and it highlights that the cross attention circuit needs to recognize and leverage word order to understand object relation. 

%Surprisingly, at each scale, DiT trained with RTEP achieved much higher spatial relation accuracy than ones trained with T5 embedding. 
%This highlights that, semantic and contextual word embedding from pre-trained language model is not necessary for DiT to learn to generate correct object relation, it could even be harmful for learning relation! 
%Further, contextual word embedding from well trained language models could even hurt the performance of image generation, esp. for spatial relation between objects.  

% \binxu{maybe mention interesting failure mode?} 
...
% If a token doesn't know the location of the other object, how does it know which object it should become? But if the tokens haven't decided what to become, how do they know the spatial relation to the other object. \binxu{polish}
% So should diffusion first decide where to generate or what to generate? 
% Given our multiple evaluations, we can specifically study which aspect of the training distribution does the model learn first. 
% We found the DiT models used to 
%During training, visually, the models first learn to generate the objects and then position them correctly with the spatial relational prompt. 
\paragraph{Training dynamics}
Having established the end-point performance trends across models, we next examine how these capabilities emerge during training. For both T5-DiT and RTE-DiT models, we evaluate models with the exponential moving average (EMA) weights \citep{Karras2024EDM2Analyzing} following common practice and the accuracy curves averaged across multiple runs are showed. We consistently observe that \texttt{color} accuracy converges first, followed by \texttt{shape} and then \texttt{unique\_binding}. \texttt{spatial\_relation} is learned the slowest (Fig.~\ref{fig:dynamics}\textbf{A.B.}), indicating that relational composition is more challenging to learn than single-object attributes or bindings. Comparing across the two text encoders, we observe that T5-DiT models converge to the final accuracy faster across all four evaluation metrics. Moreover, the temporal gaps between the convergence of different metrics are tighter. We also provide generation examples at different training steps for visual examination (Fig.~\ref{fig:dynamics}\textbf{C.}). The different learning dynamics provide the first hint that the two family of models potentially use different internal mechanisms to accomplish the same generation task. 
%, and learned visual attributes (color, shape) and feature binding nearly simultaneously during training (Fig.~\ref{fig:DiT_training_dyn_cmp}\textbf{B.C.}), which suggests, the shape and color might be learned as a combined concept in T5 model. 
 

%Consistent with diffusion-model practice, we evaluate an exponential moving average (EMA) of weights \citep{Karras2024EDM2Analyzing}: raw-model accuracy rises earlier, while EMA lags but ultimately converges slightly higher with smoother training curves (Fig.~\ref{fig:DiT_training_dyn_cmp}\textbf{A.B.}).
%Quantitatively, color accuracy saturated first, and then color shape binding accuracy, and spatial relation rised the last, consistently across all runs  (Fig.~\ref{fig:model_perf_cmp}). 
%This highlights that the object relation is indeed harder to learn, than shape and color of individual objects or feature bindings of single objects. 
% Interestingly, T5 models learn the 

%One common practice in diffusion model, is to maintained an exponential moving average (EMA) version of model weights, and use that in evaluation  \citep{Karras2024EDM2Analyzing}. 
%Regarding the effect of EMA, we noticed that the accuracy of the raw model will rise far before the EMA model, while the EMA model will converge to a slightly higher accuracy with more stable training curve (Fig.~\ref{fig:model_perf_cmp}). (see App.~\ref{} for more details.) \binxu{could put in App.}

%Now, we are ready to dissect the circuitry underlying their capability. Given the high relational performance of DiT models with RTEP embeddings, we will dissect them first, and then compare them with ones with T5 embeddings. 


% \section{Mechanism in DiT with random embedding}
% \subsection{Method: Attention Synopsis}
% \begin{figure}[!htp]
% \vspace{-4pt}
% \begin{center}
% \includegraphics[width=.9\textwidth]{figs/Figure_attn_head_finder.png}
% \end{center}
% \vspace{-8pt}
% \caption{\textbf{Attention Map Synopsis in Diffusion Transformer}. }\label{fig:attn_head_synopsis}
% \end{figure}


% \paragraph{generation dynamics}



%As a result, we summarized attention maps into summary statistics maps of shape \texttt{[num layer, num head]}, informing which attention head is involved in interactions between which type of tokens, under what condition.
%Given that the meaning and categories of the tokens are knowable, e.g. the image tokens corresponding to one object, and the text token describing the shape of the object, averaging over instances within categories can reveal interactions between categories of tokens. 
% Next we embark on the circuit mechanism that gives rise to the features. 
%PixArt DiT modularizes information interactions: self-attention exchanges information among image tokens; cross-attention injects text information into image tokens; MLPs apply per-patch nonlinearity (Fig.~\ref{fig:data_schematics}\textbf{A}).
%In PixArt DiT model, different interactions separate nicely into modules: self-attention mediates the information transmission between image tokens; cross attention mediates the influence from text to image tokens; multi-layer perceptron underpins the nonlinear operation local to one patch  (Fig.~\ref{fig:data_schematics}\textbf{A}). 
%Thus, to understand how diffusion model decide what to generate based on the prompt, we need to examine the cross attention patterns of each heads to understand the text to image information flow. %their structure and functionality. 
%Although attention-map analysis is common in ViTs and LMs \citep{caron2021emerging, yeh2023attentionviz, vaswani2017attention}, diffusion transformers add a time dimension that greatly increases map counts. For example, in DiT-B, one sample requires tens of passes, yielding a total of ([layers $\times$ heads $\times$ time steps $\times$ condition vs uncondition pass $\times$ number of tokens]) $12\times12\times10\times 2\times tokens\approx 2880\times tokens$  self- and cross-attention maps. %Across prompts and samples, manual inspection is therefore impractical.
%Many works have developed techniques to study the attention maps in language and vision transformer \citep{pan_dissecting_2024,morecite}\binxu{Panxu can add more ref}. But for diffusion transformer, the key challenge is the sheer number of them, given the additional time dimension. 
%At DiT-B, generation of a single sample involves tens of forward passes through the network, which leads to a total of ([layers x heads x time steps x condition vs uncondition pass]) $12\times12\times10\times 2\approx 2880$ cross and self-attention maps. 
%Considering variations of random seed (sample) and prompt, visually examining these attention maps becomes infeasible. 

%Here we developed a scalable paradigm to search for "useful patterns" in the attention maps, we called \textit{Attention Synopsis} (Fig.~\ref{fig:attn_head_synopsis}), which we leveraged to efficiently search through 10 million attention maps to localize the relevant circuit mechanisms for generation. 

%Our interest is cross-attention maps, that are matrices of shape (\texttt{num image tokens}, \texttt{num text tokens}). Simply averaging attention maps over different samples and prompts can obscure specific interaction patterns between certain words in the text and objects in the images. 

% For different samples and prompts, the image and text token at each index represent different meanings. Thus, averaging over attention maps will blur up the interaction pattern.  
% However, if we categorize tokens and design \textit{summary statistics} to describe the communication between \textit{types of token}, e.g. the image tokens corresponding to one object, and the text token describing the shape of the object, then this statistics will be \textit{well aligned} across prompts and samples. 
% Then, patterns shall emerge after averaging such statistics over many noise seeds and prompts. 
%After this, we will be left with a tensor of \texttt{[num layer, num head, num steps]}. 
%We noticed that the attention maps usually change smoothly across time, so we will summarize (i.e. max, mean) across time and examine temporal dynamics later. 
%As a result, we summarized attention maps into summary statistics maps of shape \texttt{[num layer, num head]}, informing which attention head is involved in interactions between which type of tokens, under what condition. 
% One such summary statistics is the communication or attention strength between two type of tokens, for example, . Then we could generate 

\begin{figure*}[!hbtp]
  \centering
  \vspace{-15pt}
  \includegraphics[width=0.99\linewidth]{figs/Figure_attn_head_finder_Final.png}
  \vspace{-8pt}
  ...
  \textbf{B.} \textbf{Weight space head screening}: We also develop an efficient weight-space screening method to screen for relation heads: computing the QK interaction between image position features and text relation feature, and scoring them by how well these spatial maps align with the reference maps corresponding to the relation.
  % \textbf{Weight space head screening}: We score each attention head by the cosine similarity between its QK interaction map (image position $W_q$ vs.\ text relation feature $W_k$) and a reference spatial gradient, screening across all 8 relations to identify potential relation heads.
  % \textbf{B.} \textbf{Weight space head screening}: To efficiently identify relation heads, we compute the QK interaction map between image position features (projected by $W_q$) and text spatial relation features (projected by $W_k$), then score each attention head by the cosine or projection similarity between this interaction map and a reference spatial gradient corresponding to the target relation (e.g., \textit{below}, \textit{lower\_left}). Heads are screened across all 8 spatial relations, and the highest-scoring head per relation is identified in the Layer $\times$ Head grid.
  }
  \label{fig:synopsis}
  \vspace{-2pt}
\end{figure*}
...
\begin{figure*}[!htbp]
  \centering
  % \vspace{-10pt}
  \includegraphics[width=0.95\linewidth]{figs/Figure_RelationHead_Final.png}
  \caption{\textbf{The spatial relation heads in random-embedding-based DiT}. \textbf{A.} We find specialized cross attention heads that contributes to the object image tokens (top: the object1 in the text; bottom: the object2 in the text) attending to the relation text tokens. \textbf{B.} We show the activation of this head across images tokens and sampling steps. The map for the composite relation “below and right” decomposes cleanly as the sum of the maps for “below” and “right”, \textbf{C.} The observed attention patterns can be induced by QK interaction of positional embedding and text embedding of relation words. \textbf{D.} Ablating relation head (L2H8) specifically affects relation evaluation.}
...
% \begin{figure}[htbp]
%   \centering
% \includegraphics[width=0.6\linewidth]{mock_figs/Slide7.png}
%   \caption{Causal}
%   \label{fig:causal1}
% \end{figure}


%—which image tokens attend to them, and how this affects generation.
%To address these questions, we analyzed the cross-attention strength from image tokens (grouped by objects) to relational words in the text.
%However, less is known about how image tokens attend to text tokens describing spatial relations \textit{between} objects and how such cross attention affects generation

% \paragraph{Phenomenology}

We first use attention synopses to look for cross attention heads that pass information from relational text tokens to image tokens. We find, indeed, there are specialized heads for this transmission. In the case of ``object1'' (i.e. the first object in the text), there is only one head (L2H8) that dominates, while in the case of ``object2'', there are a small number of heads that have this role (Fig.~\ref{fig:relation_head}A). We also find a similar pattern in models of other sizes (App.~\ref{app_sec:relation_head}). % \todo{ref Supplement figure}. 
...
% It changes from regions that represents the relation words when there is no object in the image, to localized around the object1. The pattern indicates that, in the initial phase, the query that maximizes the attention on this head is aligned with positional embedding; in the later phase, 

% Tracking attention to relational words revealed a single head—Layer 2 Head 8 (L2H8)—that consistently mediates communication between image tokens and relation tokens, but only when the image tokens belong to the first object mentioned in the prompt (\texttt{Object1}) (Fig.~\ref{fig:relation_head}\textbf{A}).
% This pattern is invariant to the first object’s color or shape, but strictly tied to its order in the prompt, raising a key question: how can an image token "know" its order in the text prompt?

% The answer lies in reversing the causal view: it's not that the image token of \texttt{Object1} pays attention to relation, it's the cross attention towards the relation word that makes the specific image token \textit{develop into} \texttt{Object1} in the end. 
% Visualizing the attention to relation words over the image grid shows a smooth, relation-specific spatial pattern even at the first diffusion step, which gradually sharpens to the final set of \texttt{Object1} tokens — explaining the invariance to object attributes (Fig.~\ref{fig:relation_head}\textbf{B}). 

% We tracked the attention pattern to the relational words in the text, and found that a single head (L2H8) consistently mediates the communication between the objects token in image and the relation words in text token, with an unexpected condition: when the object in image correspond to the first object mentioned in the prompt  (\texttt{Object1}). 
% This communication pattern is invariant to the shape or color of the object, but strictly depending on whether the object image token is the first or second in the prompt. 
% This creates a mystery, i.e. how does the image token know their identity and the position of corresponding object in the text? 

% The revelation comes when we realized that the causal relation is flipped: it's not that the image token of \texttt{Object1} pays attention to relation, it's the cross attention towards the relation word that makes the specific image token \textit{develop into} \texttt{Object1} in the end. 
% When we visualize the map of attention strength towards the relation word, we can see it lays out a spatial grid even at the first diffusion step, then the map gradually refines and focuses on the tokens that becomes the \textit{Object1} in the final sample. 
% This explains why this head is invariant towards the object properties.  

%\paragraph{Mechanism of relation Head} 

%In effect, this relational head serves as a Rosetta stone, translating the concept of "above" from the text embedding space into its corresponding spatial pattern on the image canvas.
%[\binxu{I think you probably need to explain the exact experiment in the 5*L2H8 VO figure. now the text only talks about the left side of the figure doesn't mention the right side at all}]
% We found L2H8 implementing a \textit{minimal circuit} via its QK projection, producing this behavior (Fig.~\ref{fig:relation_head}, \textbf{B}): the relation head operates by projecting the sinusoidal positional encodings in the image tokens—into the query space (Q) and relation-word embeddings into the key space (K) via learned linear layers ($W_q$, $W_k$). 
% This QK interaction aligns specific positions in the image grid with the semantics of spatial-relation tokens. The resulting map of inner product (Fig.~\ref{fig:relation_head} \textbf{C}) show smooth gradients with orientation matching the relation (e.g., "above" yields a vertical gradient). 
% These gradients effectively tag regions of the image corresponding to the first object’s intended location, which downstream heads could then use to place and render the object accurately.
% This shows that this relational head functions as a Rosetta stone translating the "above" in the textual embedding space to the "above" on the image canvas.  
% We found a minimal circuit that reproduces this behavior: the spatial-relation tokens (e.g., "above", "left") MLP projected are queried against the sinusoidal positional encodings of image tokens via the QK pathway, producing a smooth gradient pattern over spatial locations (Fig.~\ref{fig:relation_head}, middle). 
%The corresponding VO pathway writes a discrete positional "tag" into the attended image tokens, marking where the first object should be placed. This positional tag is later read by downstream heads to guide attribute assignment, enabling the model to decouple "where" from "what".
% As illustrated in the upper-right schematic of (Fig.~\ref{fig:relation_head}, B), 

% \begin{wrapfigure}[18]{r}{0.45\linewidth}
% \begin{figure}[!htbp]
% \vspace{-15pt}
%   \centering
% \includegraphics[width=0.75\linewidth]{figs/Figure_Ablation.png}
%   \caption{\textbf{Attention head ablation} reveals focused effect of L2H8 on spatial relation and L4H3 on object shape. }%\binxu{Jingxuan sent you the new version in the slack. We deleted color and object in the first plot, object and spatial in the second plot, to make it cleaner.}
%   \label{fig:ablation1}
% \vspace{-15pt}
% \end{figure}
...
The spatial relation head allows differential tagging of image tokens based on relational text tokens. To successfully generate the composite scene, the model also needs to generate the correct object on the tagged canvas. To this purpose, we examine cross attention heads that pass object shape information from text tokens to image tokens. We identify a prominently active head—Layer 4, Head 3 (L4H3) that consistently mediates communication between an object’s image tokens and its corresponding shape word in the prompt (Fig.~\ref{fig:object_head}\textbf{A}). This linkage is invariant to both the object’s position in the sentence (Shape 1 or 2) and the specified spatial relation, indicating that the head transmits shape identity independently of relational context (Fig.~\ref{fig:object_head}\textbf{A}). %(\binxu{any figure or table results supporting the claim? what fig shows it's invariant}). 
During sampling, in contrast to \textbf{spatial relation head}, this communication channel is active later in sampling (step 4-8), linking each objects to their corresponding shape tokens in the prompt (Fig.~\ref{fig:object_head}\textbf{B}).  %\binxu{I think it's maybe better to make sure all heatmaps in a figure has the same range. when I see the referred fig and compare your text, my first thought is this is not right claim this head seems to active all the time not just 4-8, but then I see that they are not on same scale.}

\subsection{Ablation and causal manipulation} 
To test whether the heads discussed above have a causal role in correct spatial relation and object shape generation, we perform both ablation and casual manipulation experiments.  
...
This confirms L2H8’s critical role in implementing the correct spatial layout. On the other hand, ablating cross-attention to the object text in specifically L4H3 decreases the object shape generation accuracy from 90\% to 76\%, while the ablation of other layer head combinations shows minimal effect (Fig.~\ref{fig:object_head}\textbf{D}). This emphasized the critical value of such a head in robustly generating the correct object shape. Although the effect size of object shape ablation is smaller than that of spatial relations ablation, the effects in both cases are confined only to the two previously identified heads, L2H8 and L4H3, suggesting a highly concentrated circuit.

We reason that L2H8 and L4H3 functions in sequence to generate a correct object at the correct spatial location. To test this hypothesis, we use the VO output of the relation head (L2H8) along with positional embeddings as the `image tokens' of the object generation head (L4H3). 
...
% \begin{wrapfigure}[27]{r}{0.48\linewidth}
% \end{wrapfigure}


% Fig.~\ref{fig:relation_head} \textbf{C.} shows that ablating \texttt{spatial} cross-attention particularly in L2H8 makes spatial relation generation accuracy decrease from 67\% to 33\% while ablation of other layer head combinations shows minimal effect. This emphasized the critical value of such a head in robustly generating correct spatial relations.
%To causally test this idea, we blocked the cross attention for L2H8, and it induced a dramatic drop in the spatial generation accuracy (to xx\% )--- the two objects are often generated as overlapping each other. 
%This emphasized the critical value of such a head in generation robustly. \binxu{Jingxuan can add the new results}

%We performed layer- and head-specific image-to-text cross-attention ablations on 3 types of text tokens - color (e.g. \texttt{red}), shape (e.g. \texttt{triangle}) and spatial relation (e.g. \texttt{left}). We then evaluated generation accuracy on object's color, shape, spatial relation and multi-property binding. Targeted ablation of the two specific layer-head circuits discovered through attention visualization leads to casual deficit in spatial relation and shape generation respectively. 

%\paragraph{Consistency}
\paragraph{Consistency across model sizes} 
Finally, we assess whether this circuit mechanism generalizes across RTE-DiT models of different sizes (App.~\ref{app_sec:relation_head}). 
...
%(\binxu{from the workshop reviewer comment: we need to show some of the same attention map figures of these other models - I think you already have them})% (App.~\ref{}). 
%\binxu{I think we also have some observations on object heads in smaller models?} 


\begin{figure}[!htbp]
\vspace{-8pt}
  \centering
\includegraphics[width=0.9\linewidth]{figs/Figure_Attn_location_circuit_diagram-02.png}
\vspace{-5pt}
...
%     \centering
%     \vspace{-10pt} % Optional: adjust vertical position
%     \includegraphics[width=1\linewidth]{figs/Figure_ObjectShapeHead.png}
%     % \caption{\textbf{Evidence for object shape head} Top: Layer$\times$head synopses of cross-attention from each object’s image token to its corresponding shape token, averaged over prompts with different colors. Consistent hotspots reveal a color-invariant small set of heads that reliably implement the image object→text shape linkage. Bottom: Ablation of the key head (L4H3) leads to selective degradation in shape accuracy, confirming its causal role.}
%     \caption{\textbf{Evidence for an object–shape head (L4H3).} \textbf{A} Layer×head synopses of cross-attention, computed from image tokens of each object to its own \emph{shape} token and averaged across prompts. A single, color-invariant hotspot appears at Layer-4 Head-3 for both object positions (\texttt{shape1}/\texttt{shape2}; blue/red controls), indicating a dedicated shape-reading head. \textbf{B} Time-resolved maps during sampling for the prompt "\enquote{red square is below and to the right of the blue circle}": tokens at the eventual square location attend to "square," while the other object attends to "circle"; selectivity sharpens from Step 0→12. \textbf{C} Minimal-circuit test: injecting the VO output of the relation head (L2H8) into positional embeddings is sufficient to elicit selective attention from tagged locations to the "square" token (left); without the tag the pattern is weak (right), showing that the shape head reads the relational tag. \textbf{D} Causal ablation across L4 heads: masking shape-token cross-attention produces a selective drop in shape accuracy only at L4H3, while masking colors or spatial tokens has little effect—confirming L4H3 as the object-shape head.}
%     \label{fig:object_generation_head}
% \end{figure}
% \par\vspace{0pt}

% \paragraph{Phenomenology} 
%A second prominent and stable pattern emerges in cross-attention from image tokens to shape-related text tokens. We identified a single head—Layer 4, Head 3 (L4H3)—that consistently mediates communication between an object’s image tokens and its corresponding shape word in the prompt (Fig.~\ref{fig:object_generation_head}\textbf{A}). This linkage is invariant to both the object’s position in the sentence and the specified spatial relation, indicating that the head encodes shape identity independently of relational context. 
%During sampling, we can see this communication channel is active later in sampling (step4-8), linking each objects to their corresponding shape tokens in the prompt (Fig.~\ref{fig:object_generation_head}\textbf{B}).  

%\paragraph{Mechanism}
%To connect this head’s role to the relational head, we injected the value–output (VO) output of L2H8 into the raw positional embeddings of the image tokens. This manipulation was sufficient to elicit selective attention to the first shape token in the prompt at the "specified locations"  (Fig.~\ref{fig:object_generation_head}\textbf{C}). This confirms that the relational head’s VO output serves as a positional "tag", and that the QK circuit in L4H3 reads these tags to determine which object to generate during denoising. Conceptually, the VO circuit of L4H3 then retrieves and translates the text concept of the shape (e.g., "square") into its corresponding visual representation in the image embedding space.

% after the relational head has already tagged spatial locations for each object. Image tokens at tagged locations attend to their respective shape tokens through L4H3, retrieving geometric identity (e.g., "square," "circle") from the text embedding. The resulting value–output (VO) projection injects shape features into the image representation, enabling the model to render the correct geometry at the correct location. In this way, the relational head provides the where, and the object generation head supplies the what.

%\paragraph{Ablation}
%Similarly, we perform layer- and head-specific ablation in image to text tokens cross-attention and evaluate image generation on all testing prompts. We found that ablating \texttt{shape} cross-attention particularly in L4H3 makes object shape generation accuracy decrease from 90\% to 76\% while ablation of other layer head combinations shows minimal effect (Fig.~\ref{fig:object_generation_head}\textbf{D}). This emphasized the critical value of such a head in robustly generating correct object shape.


% \subsection{Object Winner-Take-All Head}

%\subsection{Summary}

% \begin{wrapfigure}[22]{r}{0.47\linewidth} % 'r' for right, 'l' for left
%     \centering
%     \vspace{-45pt} % Optional: adjust vertical position
%     \includegraphics[width=\linewidth]{figs/Figure_Attn_location_circuit_diagram-02.png}
%     \vspace{-5pt}
%     \caption{\textbf{Schematics of the object relation circuit in DiT trained with random embedding}}
%     \label{fig:circuit_diagram_RTEP}
% \end{wrapfigure}
% \par\vspace{0pt}
...
\paragraph{Summary}
% Taking it together, we have a sketch of circuit mechanism responsible for relational object generation (Fig.~\ref{fig:circuit_diagram_RTEP}). 
...
%\textbf{Early stage – Relational Head (L2H8)}: 
the \textbf{``spatial relation head''} reads relational text tokens (e.g., "above," "left") via the QK circuit and interact with the sinusoidal positional encoding of image tokens, producing spatial gradients for each relation. 
...
%\textbf{Later stage – Object Generation Head (L4H3)}:
In the \textbf{``object generation head''}, tokens with matching tags attend to shape token of the corresponding object. 
...
%     \centering
%     \vspace{-10pt} % Optional: adjust vertical position
%     \includegraphics[width=0.4\linewidth]{figs/Figure_Attn_location_circuit_diagram-02.png}
%     \caption{\textbf{Schematics of the object relation circuit in DiT trained with random embedding}}
%     \label{fig:circuit_diagram_RTEP}
% \end{figure}

This modularizes operation where relation heads laying the ground and object heads assigning attributes provides a clean and disentangled mechanism for robust composition of relation and object combination. %from relational language to spatially grounded synthesis. %explaining the model’s sensitivity to relational token ablation.
...
\begin{figure*}[!htbp]
  \centering
\vspace{-15pt}
\includegraphics[width=0.99\linewidth]{figs/Figure_DiT_T5_wordvec_manipulation_Final.png}
  \caption{\textbf{Mechanism for relational generation in T5-DiT. }
\textbf{A.} T5-based DiT is robust to attention
ablation of relation word, but most sensitive to \texttt{shape2} and \texttt{EOS}. 
...
% \begin{figure}[htbp]
%   \centering
% \includegraphics[width=0.6\linewidth]{mock_figs/Slide10.png}
%   \caption{Manipulation via factorized word vector arithmetic causally affects generated object relation.}
% \end{figure}

% \begin{figure}[htbp]
%   \centering
% \includegraphics[width=0.6\linewidth]{mock_figs/Slide11.png}
%   \caption{}
%   \label{fig:robustness}
% \end{figure}

% \paragraph{Failure to find specialized cross-attention heads}

For T5-DiT, applying the same \textit{Attention Synopsis} method reveals no clear pattern for spatial relation heads (App.~\ref{app_sec:T5_DiT_attn_synops}), suggesting a different underlying mechanism.
%Thus, we seek alternative strategy for the alternative mechanism. 

% \paragraph{T5 models extract relation information from other tokes}
...
Surprisingly, the relation words, filler and color words have little effect on the generation performance (Fig.~\ref{fig:ablation2}\textbf{A}). 
...
This result suggests a different mechanism: T5's self-attention spreads the complete sentence context into many text tokens, so DiT only needs to decode the relevant information from \texttt{shape2} (and \texttt{shape1}) without relying on relation words. %This explains why T5-DiT's attention synopses are noisy.
...
We further dissect this representation by variance partitioning (App.~\ref{method:variance_partitioning}). We model the contextual embedding of the 2nd shape token as a linear combination of four factor vectors:
...
%This decomposition explains why linear operations on the shape token embedding can selectively alter relational layout while preserving other object attributes.

\begin{table}[!h] % 'r' for right, 'l' for left
\centering
% \footnotesize
\scriptsize
...
Given these results, we hypothesized that certain cross-attention heads read from the \texttt{shape2} token and specifically interact with its relation factor. To evaluate this, we calculated the QK interaction between the positional encodings of image tokens and the learned relation factor vectors $V_{rel}$, i.e., $(W_q E_{pos})^\top(W_k V_{rel})$. To be useful for relational object generation, such a head should yield a spatially modulated attention map (e.g. a gradient) that aligns with the semantics of the relation vector. We screened all heads by comparing the resulting QK inner product maps to ideal spatial gradient across the eight relations (Fig.~\ref{fig:synopsis}\textbf{B}, method detailed in App.~\ref{method:head_screening}). Notably, we identified one or two cross-attention heads that produce strong spatial gradients matching the intended relation (L3H7, Fig.~\ref{fig:ablation2}\textbf{B}). 
...
Although RTE- and T5-based DiTs achieve similar in-distribution spatial-relation accuracy, their different circuitry mechanisms suggest potential different generalization behaviors especially when the text tokens are perturbed. As detailed in \ref{app_sec:robustness}, small prompt changes, such as inserting filler word "the" before the object, sentence reversion, or using color synonym all cause a large drop in T5-DiT relation accuracy, while RTE-DiT remains much more stable (Fig.~\ref{fig:robustness_the}). 
...
% In our setup, due to a lack of text diversity, the readout direction is not perfectly aligned with the semantic direction of the relation, which can be easily disrupted by nuisance words.

% \paragraph{Robustness of RTE-based and T5-based circuits}

% When evaluating on prompt with the exact format as the training ones, RTE- and T5-trained models have comparably high performance on spatial relation. However, slight prompt variation breaks the tie, i.e. adding \textit{the} to the prompt reduces the relational accuracy of T5-DiT model by around 40\% (Fig.~\ref{fig:eval_bench_bars}\textbf{B.}, Tab. \ref{tab:model_cmp_eval_table}). This suggests that even though the task accuracies are similar between RTE-based and T5-based T2I models, their robustness to the small perturbations in the text is different. The T5-based model is more sensitive to the perturbation, likely because altered context shifts token embeddings away from the learned directions used by its cross-attention and MLP layers.

\paragraph{Transferability to pre-trained models} 

We tested whether these circuit-analysis tools transfer to a pre-trained model, PixArt-Sigma. As shown in \ref{app_sec:pretrained}, PixArt-Sigma has only weak spatial-relation generation ability: among 30 object pairs, only 8 show nontrivial object and relation accuracy. Nevertheless, token ablation indicates that spatial information is present in both the relation word and object words, which is consistent with our findings on the synthetic dataset. Head screening reveals a small number of heads that are strongly associated with spatial generation (\cref{suppfig:pretrained}). Thus, even if spatial relation performance is limited, the same analysis still identifies sparse, interpretable spatial circuits in a pre-trained model. 
...
RTE-DiTs are highly sensitive to relational words ablation yet robust to filler word changes. In contrast, T5-DiTs are robust to relational word ablation but sensitive to small lexical variations, indicating that explicit relation tokens play a limited role in their generation. These differences in generation properties can be linked to their distinctive attention circuit mechanisms. RTE-DiTs use a modularized strategy: a specialized "relational heads" directly read and translate relation tokens into spatial gradients across image tokens, guiding object placement before identity assignment; then a separate object generate head place the objects according to the spatial gradients. T5-DiTs instead use an object-centric strategy: contextual embeddings from T5 absorb relational information into object tokens (especially the last shape words), allowing both spatial layout and single object attribute to be decoded from one or two token embeddings. 
This approach is more compact but less disentangled, and sensitive to small perturbation of the two token embedding, even due to addition of nuisance words. 

%\paragraph{Mechanistic differences.}
%Interpretability analysis shows that RTE-DiTs adopt a distributed, relation-driven strategy:  
...
% We find that Diffusion Transformers can learn to generate scenes with correct object–relation composition when trained on a minimal relational dataset, but the underlying mechanisms depend strongly on the text encoder. Both T5-based and random-embedding-plus-positional-encoding (RTE) models achieve high relational accuracy, yet differ sharply in generalization, sensitivity, and circuit structure.

% % \paragraph{Generalization behavior.}
% RTE-DiTs are highly sensitive to relational words—ablating cross-attention from these tokens sharply reduces spatial accuracy—yet robust to filler word changes. In contrast, T5-DiTs are robust to relational word ablation but sensitive to small lexical variations, indicating that explicit relation tokens play a limited role in their generation.

% % \paragraph{Mechanistic differences.}
% Interpretability analysis shows that RTE-DiTs adopt a distributed, relation-driven strategy: specialized "relational heads" directly read and translate relation tokens into spatial gradients across image tokens, guiding object placement before identity assignment; then a separate object generate head place the objects according to 
% T5-DiTs instead use an object-centric strategy: contextual embeddings from T5 absorb relational information into object-related tokens (especially the last shape words), allowing spatial layout to be decoded from one or two vectors. 
% This approach is more compact but less disentangled and sensitive to small perturbation of the two token embedding, even due to addition of nuisance words. 
...
We gratefully acknowledge support from the Kempner Research Fellowship (B.W.) and the Schwartz Fellowship (X.P.), as well as computing resources provided by the Kempner Institute cluster. 
We thank Martin Wattenberg, Yonatan Belinkov, and Thomas Fel for their careful reading and valuable feedback on earlier versions of this work. We also appreciate the insightful comments and discussion from participants and reviewers of the New England Mechanistic Interpretability (NEMI) Workshop and the Mechanistic Interpretability Workshop at NeurIPS 2025. 
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=0.85\linewidth]{figs/scatter_single_vs_spatial.pdf}
...
\begin{figure}[!htp]
\begin{center}
\includegraphics[width=0.9\textwidth]{figs/Figure_training_dynamics.png}
\end{center}
\caption{
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{figs/Figure_sampling_dynamics_demo.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{figs/FigureSupp_Attn_dynamics-01.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.7\textwidth]{figs/FigureSupp_Attn_dynamics-03.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.85\textwidth]{figs/FigureSupp_RelationObjectHead_DiT-mini_relhead.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=1\textwidth]{figs/FigureSupp_RelationObjectHead_DiT-mini_relhead_inv.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.85\textwidth]{figs/FigureSupp_RelationObjectHead_DiT-micro_relhead.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=1\textwidth]{figs/FigureSupp_RelationObjectHead_DiT-micro_relhead_inv.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{figs/FigureSupp_RelationObjectHead_DiT-mini_object_head.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.99\textwidth]{figs/FigureSupp_RelationObjectHead_DiT-micro_objecthead.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=0.85\textwidth]{figs/FigureSupp_RelationObjectHead_DiT-B_T5_relhead.png}
\end{center}
...
\begin{figure}[!htbp]
\begin{center}
\includegraphics[width=1\textwidth]{figs/FigureSupp_RelationObjectHead_DiT-B_T5_relhead_inv.png}
\end{center}
...
\begin{figure}[!htp]
\begin{center}
\includegraphics[width=0.99\textwidth]{figs/Figure_DiT_T5_wordvec_vis_UMAP.png}
\end{center}
\caption{
...
\begin{figure}[!htp]
\begin{center}
\includegraphics[width=0.95\textwidth]{figs/FigureSupp_HeadScreening_all_models_RTE-DiT.png}
\end{center}
\caption{
...
\begin{figure}[!htp]
\begin{center}
\includegraphics[width=0.95\textwidth]{figs/FigureSupp_HeadScreening_all_models_T5-DiT_shape2.png}
\end{center}
\caption{
...
\begin{figure}[!htp]
    \begin{center}
    \includegraphics[width=0.95\textwidth]{figs/FigureSupp_HeadScreening_all_models_T5-DiT_shape1.png}
    \end{center}
    \caption{
...
\begin{figure}[!htp]
    \begin{center}
    \includegraphics[width=0.95\textwidth]{figs/FigureSupp_HeadScreening_all_models_CLIP-DiT.png}
    \end{center}
    \caption{
...
\begin{center}
\vspace{-6pt}
\includegraphics[width=0.6\linewidth]{figs/Data_schematics_Final_Eval.png}
\vspace{-8pt}
\end{center}
\caption{
\textbf{Evaluation of model performance on trained and generalized prompt template.} 
...
\begin{figure}[!htp]
  \centering
\vspace{-4pt}
  \includegraphics[width=0.85\linewidth]{rebuttal_figs/rebuttal_figures_the_robust.png}
% \vspace{-5pt}
...
\begin{figure}[!htp]
    \begin{center}
    \includegraphics[width=0.99\textwidth]{figs/FigureSupp_prompt_generaization_cmp.png}
    \vspace{-10pt}
    \end{center}
...
\begin{figure}[!htp]
    \centering
    \vspace{-5pt}
    \includegraphics[width=0.90\linewidth]{figs/Figure_CLIP_DiT_supplementary.png}
    \vspace{-5pt}
  ...
  \begin{figure}[!htp]
    \centering
    % \vspace{-13pt}
    \includegraphics[width=0.93\linewidth]{figs/pretrained.png}
    % \vspace{-5pt}
...
\]
We additionally construct an ``unconditional'' prompt by tokenizing the empty string $c = ""$, which is used as the null-conditioning input (e.g., for classifier-free guidance). %The utility \texttt{tokenize\_embed\_prompt} returns both unconditional and conditional prompt embeddings and their corresponding token IDs.
...
\[
\texttt{input\_ids2dict\_ids} : \mathbb{N} \rightarrow \{0,\ldots,V'-1\}, \qquad
